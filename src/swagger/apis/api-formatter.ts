...
export function format_controllers(
  ...
  for (const [controller_name, controller_file] of controller_iter) {
    ...
    const file_content = import_lines.concat(server_URL).concat(formatted_endpoints);
...
function get_response_type(response_type: SCHEMA_TYPE | null): string {
  if (response_type == null) return 'any';
...
export function build_query_params(queryParams: any[]): string {
  ...
  const rez = ' + `?' + quarryParamArray.join('&') + '`';
...
export function build_fn_body(endpoint: ENDPOINT_schema, ds_conf: DataSourceConfig): string {
  ...
  const query_params = endpoint.params.query.length > 0 ? '?${params.toString()}' : '';
  let call_params = `URL + \`${path}${query_params}\``;

  if (request_body_type !== null) call_params += `, body`;

  const query_build =
    endpoint.params.query.length > 0
  ...
  return `${query_build} return ${http_service_call}`;
}
