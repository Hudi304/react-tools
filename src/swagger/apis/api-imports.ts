...
import { getSchemaType, isModel } from '../common/utils'
...
function get_endpoint_imports(
  ...
  query_params.forEach((q_param) => {
    const import_type = getSchemaType(q_param.schema)

    if (import_type.isPrimitive) return

    const enum_to_be_imported = enum_map.get(import_type.type)

    if (!enum_to_be_imported) return

    const import_line: IMPORT_schema = {
      name: enum_to_be_imported.name,
      path: enum_to_be_imported.filePath,
      fileType: FILE_TYPE.ENUM,
    }
